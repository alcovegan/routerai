...
- Пиннинг провайдера через строку модели (`route(model, provider=...,
  allow_fallbacks=False)`) и алиасы моделей (`~разработчик/линейка-latest`), которые
  указывают на свежий релиз; расход при этом остаётся записан на запрошенную модель
...
### Роутинг провайдера и алиасы моделей

В строке модели едут две вещи разом. Суффикс `@` прикрепляет запрос к
конкретному провайдеру, не трогая тело запроса; ведущий `~` помечает алиас,
который всегда указывает на самый свежий релиз в линейке.

```python
from routerai import route

# Прикрепить провайдера. allow_fallbacks=False превращает молчаливое
# переключение на другого провайдера в NotFoundError — ради этого пиннинг
# и нужен.
...
# Алиас подхватывает новый релиз без правки конфигов на вашей стороне.
client.chat.complete("~anthropic/claude-sonnet-latest", "привет")

client.models.aliases()                             # все алиасы каталога
client.models.resolve("~anthropic/claude-sonnet-latest")   # на что указывает сейчас
client.models.search("claude", aliases="exclude")   # только конкретные релизы
```

Что стоит знать, прежде чем на это опираться:

- Суффикс `@` понимают только `chat.complete`/`chat.stream`. Остальные
  эндпоинты читают всю строку как имя модели и отвечают
  `400 Model '...@provider=x' not found`.
- Задать `provider` и в строке, и в аргументе `provider=` — это `400` от
  сервера. SDK поднимает `ConfigurationError`, не тратя запрос.
- Алиас помечен префиксом `~` и ничем больше. `openai/gpt-chat-latest` —
  обычная модель, несмотря на суффикс.
- Обратной ссылки на цель у алиаса в каталоге нет: запись устроена точно так
  же, как любая другая. Поэтому `resolve()` сопоставляет по полю `name`,
  которое алиас наследует от релиза, за который стоит.
- `cheapest()` по умолчанию алиасы не рассматривает: алиас несёт цену своей
  цели, и включение алиасов лишь добавляет ничью, победитель которой зависит
  от сортировки идентификаторов. Нужен «плавающий» id — передайте
  `aliases="include"`.
- Расход записывается на ту модель, которую вы запросили, а не на имя из
  ответа сервера. Расходятся они чаще, чем кажется: алиас возвращается уже
  разрешённым, а некоторые провайдеры отвечают коротким именем, которого нет
  ни в каком каталоге (`deepseek/deepseek-v4-flash-0731` обслуживается как
  `deepseek-v4-flash`). В `UsageRecord.model` при этом лежит то, что сказал
  сервер.
